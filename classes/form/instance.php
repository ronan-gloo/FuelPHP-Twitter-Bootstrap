...
class Form_Instance extends \Fuel\Core\Form_Instance {
	...
	{
		static::$helper = Bootstrap::forge('form');
	}
	...
	/**
	 * Generate a control-grouo type div open.
	 * 
	 * @access public
	 * @static
	 * @param array $attrs (default: array())
	...
	protected function checkbox_and_radio($type, $field, $value, $checked, $attrs)
	{
		...
		if (array_key_exists('label', $attrs))
		{
			...
			$output = $prep."\n".html_tag('label', array('class' => $class), $input."\n".$text);
		}
		else
		{
	...
	/**
	 * Check arguments for prepend / append and build the html
	 * if something like 'icon- is paased as value, we build
	 * the icon tag.
	 * 
	 * @access protected
	 * @return void
